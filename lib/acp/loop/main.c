...
#define ACPL_CHECK_BUSY_TIME if (ton(&self->busy_tmr)) {return ACP_BUSY;}
...
inline void acpl_prepWrite(Acpl *self){
	self->wi = 0UL; 
	self->wbuf_size = strlen(self->buf) * sizeof (*self->buf);
}

inline void acpl_prepRead(Acpl *self){
	memset(self->buf, 0, sizeof (*self->buf) * ACP_BUF_MAX_LENGTH); 
	self->len = 0; 
	ton_reset(&self->pack_tmr);
}
