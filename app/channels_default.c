 ...
 * id,
 * enable: YES or NO,
 * display_kind: DEVICE_KIND_MAX7219 or DEVICE_KIND_TM1637 or DEVICE_KIND_DSERIAL or DEVICE_KIND_DSLED,
...
static void channels_buildFromArray(){
	if(CHANNEL_COUNT <= 0) return;
	channels.length = CHANNEL_COUNT;
	channels.top = &channel_buf[0];
	channels.last = &channel_buf[CHANNEL_COUNT - 1];
	for(size_t i = 0; i<CHANNEL_COUNT; i++){
		channel_buf[i].next = NULL;
	}
	for(size_t i = 0; i<CHANNEL_COUNT-1; i++){
		channel_buf[i].next = &channel_buf[i+1];
	}
}

void channels_begin(int default_btn){
	channels_buildFromArray();
	size_t ind = 0;
	FOREACH_CHANNEL {
		if(default_btn == BUTTON_DOWN){
			pmem_saveChannelParam(&CHANNEL_DEFAULT_PARAMS[ind], ind);
			printd("\tdefault param saved\n");
		}
