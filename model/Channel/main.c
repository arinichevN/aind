...
static void channel_CLIENT_ENUM_GET(Channel *self);
...
static const char *channel_intToStrError(int v);

static const char *channel_intToStrState(int v);

static const char *channel_intToStrNothing(int v);

static void channel_parseNshowSpyEnum(Channel *self, char *buf);
...
state_t channel_getState(Channel *self){
	if(self->control == channel_CLIENT_FTS_GET || self->control == channel_CLIENT_ENUM_GET || self->control == channel_CLIENT_WAIT || self->control == channel_SPY || self->control == channel_SERVER){ 
		return STATE_RUN;
...
static err_t channel_setParam(Channel *self, size_t ind){
	...
	self->intToStr = channel_intToStrNothing;
	switch(self->acp_command){
		...
		case CMD_NOID_GET_STATE:
			self->intToStr = channel_intToStrState;
			self->parseNshowSpyData = channel_parseNshowSpyEnum;
			self->parseNShowSpyRequestData = channel_parseNShowSpyRequestSkip;
			break;
		case CMD_NOID_GET_ERROR:
			self->intToStr = channel_intToStrError;
			self->parseNshowSpyData = channel_parseNshowSpyEnum;
			self->parseNShowSpyRequestData = channel_parseNShowSpyRequestSkip;
			break;
...
static const char *channel_intToStrError(int v){
	return getErrorStr((err_t) v);
}

static const char *channel_intToStrState(int v){
	return getStateStr((state_t) v);
}
...
static void channel_parseNshowSpyEnum(Channel *self, char *buf){
	int v;
	if(!acp_packGetCellI (buf, ACP_RESPONSE_IND_PARAM1, &v)){
		channel_printError(self, CHANNEL_MSG_PARSE_FAILED);
		return;
	}
	channel_printStr(self, self->intToStr(v));
}
...
static void channel_setControlFunction(Channel *self){
	...
	switch(self->mode){
		...
		case CHANNEL_MODE_CLIENT:
			switch(self->acp_command){
				case CMD_NOID_GET_FTS:		self->control = channel_CLIENT_FTS_GET; self->control_next = channel_CLIENT_FTS_GET; return;
				case CMD_NOID_GET_STATE:	self->control = channel_CLIENT_ENUM_GET; self->control_next = channel_CLIENT_ENUM_GET; return;
				case CMD_NOID_GET_ERROR:	self->control = channel_CLIENT_ENUM_GET; self->control_next = channel_CLIENT_ENUM_GET; return;
			}
			break;
...
static void channel_CLIENT_ENUM_GET(Channel *self){
	int out;
	int r = acplcm_getII(self->serial_client, self, self->acp_command, self->remote_id, &out);
	switch(r){
		case ACP_WAIT:
		case ACP_BUSY:
			break;
		case ACP_DONE:
			channel_printStr(self, self->intToStr(out));
			ton_reset(&self->tmr);
			self->control = channel_CLIENT_WAIT;
			printd(self->id); printd(" str: "); printdln(out);printdln(" ");
			break;
...
#define CHANNEL_SERVER_PRINT_CONDITION self->mode == CHANNEL_MODE_SERVER && (self->control == channel_SERVER || self->control == channel_SERVER_FAILED)
...
dk_t channel_getDisplayKind(Channel *self){
	return self->display.kind;
}

int channel_getDisplayP1(Channel *self){
	return self->display.p1;
}

int channel_getDisplayP2(Channel *self){
	return self->display.p2;
}

int channel_getDisplayP3(Channel *self){
	return self->display.p3;
}
